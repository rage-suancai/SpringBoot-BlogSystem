package com.yxs.constants;

/**
 * @author YXS
 * @PackageName: com.yxs.constants
 * @ClassName: SystemConstants
 * @Desription:
 */
public class SystemConstants {

    public static final int ARTICLE_STATUS_DRAFT =  1; // 文章是草稿

    public static final int ARTICLE_STATUS_NORMAL = 0; // 文章是正常发布状态
    public static final String STATUS_NORMAL = "0";

    public static final String LINK_STATUS_NORMAL = "0"; // 友链状态为审核通过

    public static final String ARTICLE_COMMENT = "0"; // 评论类型: 文章评论
    public static final String LINK_COMMENT = "1"; // 评论类型: 友联评论

    public static final String VIEW_COUNT = "article:viewCount"; // redis的key

    public static final String MENU = "C";
    public static final String BUTTON = "F";

    public static final String NORMAL = "0"; // 正常状态
    public static final String ADMAIN = "1";

}
